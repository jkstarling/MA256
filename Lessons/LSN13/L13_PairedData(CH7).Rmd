...
SE & \sol{$\sqrt{\frac{\pi(1-\pi)}{n}}$} & 
      \sol{$s/\sqrt{n}$} & 
      \sol{$\sqrt{\hat p(1- \hat p)\left(\frac{1}{n_1} + \frac{1}{n_2}\right)}$} &
      \sol{$\sqrt{\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}}$} \\  \hline
stand. stat & \sol{$\frac{\hat p - \pi}{sd}$} & \sol{$\frac{\bar x - \mu_0}{sd}$} & \sol{$\frac{\hat p_1 - \hat p_2- \pi_0}{sd}$} & \sol{$\frac{\bar x_1 -\bar x_2 - \mu_0}{sd}$} \\  \hline
...
CI & \sol{$\hat p \pm z^*_{(1-\alpha/2)} \cdot sd$} & 
\sol{$\bar x \pm t^*_{(1-\alpha/2, n-1)} \cdot sd$} & 
\sol{$\hat p_1 - \hat p_2 \pm z^*_{(1-\alpha/2)} \cdot \sqrt{\frac{\hat p_1 (1-\hat p_1)}{n_1} + {\frac{\hat p_2 (1-\hat p_2)}{n_2}}}$} & 
\sol{$\bar x_1 - \bar x_2 \pm t^*_{(1-\alpha/2, n-2)} \cdot sd$}  \\  \hline
\end{tabular}
...
\sol{The samples of the first E. coli count are dependent of the samples of the second E. coli count because they came from the same filter. If the filter is functioning well, we would expect both E.coli counts to be low. If the filter is not functioning well, we would expect both E. coli counts to be higher. So we will perform a repeated measures paired test for the mean difference.}
