...
We are going to determine if an association exists between `spr\_raw` and `mdl\_raw` scores.
...
## Does adding the variable `sex` change the association between the Standing Power Throw (`spr\_raw`) and Leg Tuck (`ltk\_raw`) raw scores?

Now, we add `sex` as another variable to the model but also include it as an interaction with `spr\_raw`.
...
15.  Does there appear to be a different relationship between `mdl\_raw` and `spr\_raw` based on `sex`?
...
20.  Interpret the base slope term for `spr\_raw`.
...
-   The `mdl\_raw` for a male cadet with an `spr\_raw` of 6m:

```{r}
#
```

-   The `mdl\_raw` for a female cadet with an `spr\_raw` of 6m:
