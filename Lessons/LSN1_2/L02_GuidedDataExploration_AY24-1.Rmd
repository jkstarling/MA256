...
# setwd("C:/Users/james.starling/OneDrive - West Point/Teaching/MA256-jks/MA256_AY24-1/Lessons/LSN2/")
...
So you have just graduated from USMA, completed your BOLC, and have arrived at Fort Campbell, KY. You would like to buy a house that is within your means but plan on buying one that is big enough to rent out to some of your army buddies in order to make some additional cash. We want to look at the return on investment (ROI) percentage, which is the ratio of the (monthly) cash flow divided by the purchase price. 
...
Closing_cost <- 5000
TN_eff_tax <- 0.0064
TN_homeowner_ins_mon <- 1233/12
BAH_Campbell_O1M_mon <- 1623
MPR <- 0.06/12 #Monthly Percentage Rate is the assumed Annual 3% Rate divided by 12 months
n <-360
Room_rent <-650
```
...
The HOA/MONTH column in `data` has missing (NA) values. Any time a formula includes a variable with a NA value, it results in an NA value. In this case, NA means there are no HOA fees so to fix this, we need to replace all NA values with \$0 so we can use HOA/MONTH in mathematical formulas for total monthly cost. We do this by creating a dataframe so we can use the `is.na()` function to replace all NA values with 0:
...
Create new columns that calculate the cost to buy, monthly payments, and rent. To calculate the mortgage payment per month, use the following equation:
...
house_data = house_data %>% 
  mutate(CostToBuy = 0.2*PRICE+Closing_cost, 
         Monthly_PMT = `HOA.MONTH` + TN_homeowner_ins_mon + TN_eff_tax * PRICE / 12 +
          (0.8*PRICE * (MPR*(1+MPR)^n))/((1+MPR)^n - 1),
...
```{r}
house_data = house_data %>% 
  mutate(Cash_Flow = Rent - Monthly_PMT + BAH_Campbell_O1M_mon, #BAH is all profit if rent pays for everything 
...
NOTE: If your Annual ROI is in scientific notation, you may convert it using the following code to make the output easier to interpret:
...
We see a trend within categories based on the number of bedrooms, perhaps because the maximum rental income is capped based on the number of bedrooms so the more expensive a 3BR home is, the less ROI.

Note: to overlay the plot with the best fitting least squares regression lines we must remove the #hashtag.
...
I once bought a 3BR/4BA home valued at \$244k and invested \$44k over the next month to
...
When I had a bedroom addition completed, it cost me \$15,000 so all 3 of these scenarios seem profitable, but if I could only pick one, the \$95,638 seems best.
...
1) Run the code above again, but vary a few of the numbers in the `parameters` code chunk above. Does it change any of the analysis? Which parameter will have the largest affect on ROI and cashflow?

2) Find your own Redfin and attempt to draw out your own insights. Conduct the same analysis we did above, and translate your data into meaningful information and insights to inform your our house purchase!Often, good data scientists take the code of others (appropriately cited!) and then modify to fit their needs/ purposes! To complete this lesson, use this markdown file as a guide to produce similar plots and visualizations However, the true value will come from the insights you find from the visualizations. The markets have certainly changed since this data was pulled from Redfin, and the underlying assumptions we made for that place and that time might not be the same where you want to live!
