...
house_data = house_data %>% #Again, overwrite original data so it includes new variables
  mutate(CostToBuy=0.2*PRICE+Closing_cost, #20% down + estimated $5k in closing costs
         Monthly_PMT=`HOA.MONTH`+TN_homeowner_ins_mon+TN_eff_tax*PRICE/12+
           ((0.8*PRICE*(MPR*((1+MPR)^Mortage_dur_mon)))/(((1+MPR)^Mortage_dur_mon)-1)),#HOA/mo+Ins/mo+Taxes/mo+Mortgage PMT/mo
         Rent=(BEDS-1)*Room_rent) #BEDS-1 because you'll live in one of the rooms
```
