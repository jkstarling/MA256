...
\sol{Answers will vary; these are statistics, a summary of our sample, it is what we saw in our data.}
...
```{r}
c(mean(GW$wordlen), sum(GW$cont.e) / length(GW$cont.e))
```
...
```{r out.width="40%"}
GW %>% ggplot(aes(x=wordlen)) + geom_histogram(binwidth = 1)
c(median(GW$wordlen), mean(GW$wordlen))
```
...
```{r}
set.seed(256) # use your second favorite number here (not 256)
ind <- sample(1:268, 10)
ind
mean(GW$wordlen[ind])
sum(GW$cont.e[ind]) / length(ind)
```
\sol{Answers will vary, but should be closer to the population mean of 4.287 and population mean of 0.466}
